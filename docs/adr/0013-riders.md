...
- *(Amended 2026-09-23, self-review.)* `values` no longer requires `indexed: true`. The rule dated from when only indexed fields were queryable; a value union narrows an unindexed field's filters just the same. So following the build's "barely prunes, consider removing indexed" advice never breaks the config, and the advice names any `endsWith`/`contains` that must go with the index.
- *(Amended 2026-09-23.)* `findMany({ scan: "block-order", limit })` accepts a rider-only `where`: it walks the data files in sort order and stops once the page is full, so it needs a `limit` and no `orderBy` but the sort field (a missing `limit` is a compile error, a bad `orderBy` a runtime `NEEDS_PRUNING`). It is opt-in rather than implied by `limit`, because a rider few records match still reads most of the dataset, and that cost should be visible where the query is written. It replaces the full-range tricks (`startsWith: ""`, `gte: ""`) apps used to get past the rule.
