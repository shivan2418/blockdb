...
3. **Operators follow the data precisely.** On an indexed, single-valued, non-sort field, `nullable` unlocks `isNull`, `absent` unlocks `isAbsent`, and either unlocks `exists`. They're listed in the field's `operators` like any other, so the runtime types no longer special-case a flag. The sort field and list fields get none: their missing values have their own rules (ADR-0002 §9, ADR-0010).
