...
import { build } from "blockdb-cli";
import type { BlockDbConfig } from "blockdb-cli";
import { createClient } from "../src/client.js";
import { BlockDbError } from "../src/errors.js";
import type { SchemaMeta } from "../src/types.js";

// Same fixture shape as blockdb-cli's seam #1 test — the point of seam #2
...
beforeEach(() => {
  tmpDir = mkdtempSync(path.join(tmpdir(), "blockdb-seam2-"));
...
const sortPkConfig: BlockDbConfig = {
  ...config,
...
const secondaryPkConfig: BlockDbConfig = {
  ...config,
  schema: {
    sortField: "year",
    pk: "title",
...
describe("seam #2 — runtime failure contract & maxResults guardrail (T5), over a seam #1-built fixture tree", () => {
  /** Rejects with a BlockDbError; asserts the exact code + payload, then returns it for message checks. */
  ...
  test("a network-level rejection → NETWORK with NO .status and the original error chained as .cause", async () => {
    ...
    expect((error as BlockDbError).message).toMatch(/socket hangup/);
  });
  ...
  test("a 2xx block body that won't parse → CORRUPT_DATA with the parse error chained", async () => {
    ...
    expect((error as BlockDbError).url).toBe(path.join(outputDir, "blocks", `${victim.hash}.ndjson`));
...
describe("seam #2 — gzip block payloads (T13, ADR-0002 §8)", () => {
  test("findMany transparently decompresses gzip block payloads end-to-end over a real build", async () => {
    const gzipConfig: BlockDbConfig = { ...config, gzip: true };
