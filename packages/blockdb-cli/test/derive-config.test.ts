...
import type { FieldConfig, BlockDbConfig } from "../src/types.js";

const base = (fields: Record<string, FieldConfig>): BlockDbConfig => ({
  collection: "cards",
...
describe("--reinfer preserves derived fields", () => {
  ...
  test("carries the derive declaration over, and drops it only when its source is gone", async () => {
    ...
    const dir = mkdtempSync(nodePath.join(tmpdir(), "blockdb-derive-"));
    try {
      const configPath = nodePath.join(dir, "blockdb.config.json");
      writeFileSync(
