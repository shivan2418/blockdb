...
function computeEstimateForState(data: WizardData, state: WizardState): WizardEstimate {
  ...
  if (sortLocality !== undefined && sortLocality.mean > SCATTERED_SORT_FIELD_RATIO) {
    ...
    warnings.push(
      `blockdb: sorting by "${state.sortField}" scatters the fields you filter on — a typical query would read ` +
  ...
  for (const [name, idx] of Object.entries(costs.indexes)) {
    if (idx.containsExceedsColumn) {
      warnings.push(
        `blockdb: "${name}"'s contains index would be bigger than the field's own data — usually not worth it (ADR-0003 §7).`,
      );
    }
  }
...
function renderReview(state: WizardState, estimate: WizardEstimate, configPreview?: string): string[] {
  ...
  lines.push("", color(ANSI.cyan, "  [Enter] write blockdb.config.json") + dim("   [←] back"));
  return lines;
}
...
export function renderFrame(
  ...
  const header = `${bold(color(ANSI.cyan, "blockdb init"))}\n${crumbs}\n`;
