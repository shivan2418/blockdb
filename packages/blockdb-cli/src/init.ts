...
const CONFIG_SCHEMA_REF = "node_modules/blockdb-cli/config.schema.json";

export interface InitOptions {
  /** Directory input/config-relative paths resolve against. */
  cwd: string;
  /** Absolute path to read/write `blockdb.config.json`. */
  configPath: string;
...
function applyFieldFlagOverrides(
  ...
  for (const [flagName, names] of flagGroups) {
    for (const name of names ?? []) {
      if (!fields[name]) {
        throw new Error(
          `blockdb: --${flagName} "${name}" is not declared in the baked schema — pass --reinfer to rediscover fields`,
        );
      }
    }
  }
...
function duplicateValueUnionHints(fields: Record<string, FieldConfig>): string[] {
  ...
  return [...groups.entries()]
    ...
    .map(
      ([key, names]) =>
        `blockdb: ${names.join(", ")} all have the same value set [${key.split("\u0000").join(", ")}], so codegen ` +
...
function dropQueryFlagsFromJsonFields(fields: Record<string, FieldConfig>): {
  ...
  for (const [name, field] of Object.entries(fields)) {
    ...
    next[name] = cfg;
    warnings.push(
      `blockdb: field "${name}" holds nested or mixed-type values, so it is payload-only (kind "json") — ` +
...
export function resolveInitConfig(opts: InitOptions): InitResult {
  if (!opts.yes) {
    throw new Error(
      'blockdb: "init" requires --yes to run non-interactively — pass --yes plus flags, or re-run in a real terminal for the interactive wizard',
    );
  }
 ...
 * `blockdb.config.json`. `init --yes` + flags is fully scriptable; the interactive wizard
