...
export const TOP_LEVEL_HELP = `blockdb <command> [options]

  Query a large dataset from any static host: no backend,
  no WASM, no HTTP Range requests.

Commands
  init [input]              Infer a schema and write blockdb.config.json
  build                     Block + index the data, regenerate the typed client
  inspect                   Report sizes, costs and warnings without rebuilding

Options
  --config <path>           Config file (default: blockdb.config.json)
  -h, --help                Show this help
  -v, --version             Show the installed version

Run \`blockdb <command> --help\` for a command's own options.

Typical first run:
  blockdb init data/movies.ndjson     # guided wizard, writes the config
  blockdb build                       # → public/blockdb/ + src/blockdb/`;

const INIT_HELP = `blockdb init [input] [options]

  Infers a schema from your data and writes blockdb.config.json. In a real
...
Output
  ...
  --client-out <dir>        Generated client directory (default: src/blockdb)
...
const BUILD_HELP = `blockdb build [options]
...
const INSPECT_HELP = `blockdb inspect [options]
...
Options
  --config <path>           Report from a config, materializing the tree in
                            memory without writing it (default: blockdb.config.json)
